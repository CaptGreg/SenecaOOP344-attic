...
DESCRIPTION
       ...
       requires an argument, so getopt() places a pointer to the following text in the same argv-element, or the text of  the  fol‐
       lowing  argv-element, in optarg.  Two colons mean an option takes an optional arg; if there is text in the current argv-ele‐
       ment (i.e., in the same word as the option name itself, for example, "-oarg"), then it  is  returned  in  optarg,  otherwise
       optarg is set to zero.  This is a GNU extension.  If optstring contains W followed by a semicolon, then -W foo is treated as
       the long option --foo.  (The -W option is reserved by POSIX.2 for implementation extensions.)  This behavior is a GNU exten‐
       sion, not available with libraries before glibc 2.
       ...
       '-', then each nonoption argv-element is handled as if it were the argument of an option with character code  1.   (This  is
       used  by  programs that were written to expect options and other argv-elements in any order and that care about the ordering
       of the two.)  The special argument "--" forces an end of option-scanning regardless of the scanning mode.
       ...
       ment, it returns '?' and sets the external variable optopt to the actual option character.  If the first character  (follow‐
       ing  any  optional  '+'  or  '-' described above) of optstring is a colon (':'), then getopt() returns ':' instead of '?' to
       indicate a missing option argument.  If an error was detected, and the first character of optstring is not a colon, and  the
       external variable opterr is nonzero (which is the default), getopt() prints an error message.
   ...
   getopt_long() and getopt_long_only()
       ...
       may  be  abbreviated  if  the abbreviation is unique or is an exact match for some defined option.  A long option may take a
...
RETURN VALUE
       ...
       returned.  If getopt() encounters an option with a missing argument, then the return value depends on the first character in
       ...
       they  return  val  if  flag  is  NULL,  and 0 otherwise.  Error and -1 returns are the same as for getopt(), plus '?' for an
...
ENVIRONMENT
       ...
       _<PID>_GNU_nonoption_argv_flags_
              ...
              and  so  should  not  be  considered  as options.  This behavior was removed in bash(1) version 2.01, but the support
              remains in glibc.
...
CONFORMING TO
       getopt():
              ...
              either  <unistd.h>  or <stdio.h>.  POSIX.1-2001 marked the use of <stdio.h> for this purpose as LEGACY.  POSIX.1-2001
              does not allow the declaration to appear in <stdio.h>.
...
NOTES
       ...
       extensions such as '+' and '-' at the start of optstring, or changes the value of POSIXLY_CORRECT between scans, must reini‐
...
BUGS
       The POSIX.2 specification of getopt() has a technical error described in POSIX.2 Interpretation 150.  The GNU implementation
       (and probably all other implementations) implements the correct behavior rather than that specified.
...
*/
       ...
       int
       main_getopt(int argc, char *argv[])
       {
           int flags, opt;
           int nsecs, tfnd;
           ...
           while ((opt = getopt(argc, argv, "nt:")) != -1) {
               ...
               default: /* '?' */
                   ...
                   cerr << "Usage: " << argv[0] << " [-t nsecs] [-n] name\n";
                   return EXIT_FAILURE;
               }
           }
           ...
           cout << "name argument = " << argv[optind] << "\n";
...
*/
       ...
       int
       main_getopt_long(int argc, char **argv)
       {
           int c;
           int digit_optind = 0;
           ...
           while (1) {
               ...
               case 'c':
                   ...
                   cout << "option c with value '" << optarg << "'\n";
                   break;

               case 'd':
                   // printf("option d with value '%s'\n", optarg);
                   cout << "option d with value '" << optarg << "'\n";
                   break;
...
int main(int argc, char **argv)
{
    ...
    cout << "parsing args with getopt(argc, argv, \"nt:\")\n";
    main_getopt(argc, argv);
    cout << "\n";

    cout << "parsing args with getopt_long(argc, argv, \"abc:d:012\" ...\n";
    main_getopt_long(argc, argv);
    cout << "\n";
}
